...
function Invoke-IcingaCheckHTTPStatus()
{
    ...
    $HTTPAllStatusPackage = New-IcingaCheckPackage -Name "HTTP Status Check" -OperatorMin $Minimum -Verbose $Verbosity;

    foreach ($SingleUrl in $Url) {

        $TransformedUrl = $SingleUrl  -Replace "[:\./]", "_" -Replace "(_)+", "_";
        # Full Package
        $HTTPStatusPackage = New-IcingaCheckPackage -Name "HTTP Status Check $SingleUrl" -OperatorAnd -Verbose $Verbosity;
        ...
        if ($HTTPData.$SingleUrl['RequestTime'] -eq $null) {
            ...
            $HTTPResponseTimeCheck.WarnOutOfRange($Warning).CritOutOfRange($Critical);
        }
